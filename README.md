...
HARVESTER_API_KEY=
HARVESTER_ALLOWED_ORIGINS=http://127.0.0.1:5173,http://localhost:5173
...
本地使用可不设置 `HARVESTER_API_KEY`；服务器部署必须设置。启用后所有 `/api`
请求都需要 `X-Harvester-Key`。登录时可选择“记住此浏览器”；勾选后密钥会
保存在浏览器本地存储中，未勾选则只在当前标签页会话中保留。
`HARVESTER_ALLOWED_ORIGINS` 是逗号分隔的前端来源白名单。打开本地归档目录默认
只允许回环地址请求，远程部署不会触发服务器文件管理器。
